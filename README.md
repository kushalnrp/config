...
Create a flat JSON file at `config.seed.json` (or point `CONFIG_SEED_PATH` elsewhere). All key/value pairs are upserted into the DB on every startup. See `config.example.json` for the full set of keys this platform's services expect (copy it to `config.seed.json` and fill in the placeholder values):

```bash
cp config.example.json config.seed.json
./config-server
```
...
docker run -p 2001:2001 -v $(pwd)/config.seed.json:/app/config.seed.json:ro config-server
```
